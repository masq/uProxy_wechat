...
var weChatClient = function() {
  ...
  this.events.onUUID = function(url) { console.log("oops!"); return; };
  ...
  return new Promise(function(resolve, reject) {
    https.get(url, function(response) {
      ...
      response.on("end", function() {
        ...
        this.events.onUUID(this.LOGDOM + "/qrcode/" + uuid);
        resolve(uuid);
      }.bind(this));
    }.bind(this)).on("error", this.handleError);
  }.bind(this));
...
weChatClient.prototype.serveQR = function(uuid) {
  this.getQR(uuid).then(function(imgQR) {
    this.handlers["onQRCode"](imgQR, uuid); //TODO: check if this works :TODO
...
weChatClient.prototype.makeURL = function(domain, path, params, cook, postDataLen) {
  ...
  if (!cook) {
    ...
    var result;  // TODO: make cleaner/refactor if statements.
    if (this.cookies) {
      result =  {
        "hostname": domain,
